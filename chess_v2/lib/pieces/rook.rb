...
class Rook < Piece
  ...
  def rank_moves(rank)
    index = @location[1]
    empty_ranks = move_increase(index + 1, rank) + move_decrease(index - 1, rank)
  ...
  def file_moves(board)
    ...
    empty_files = move_increase(rank + 1, row) + move_decrease(rank - 1, row)
    empty_files.product([file])
  end

  def move_increase(index, row)
    empty_increase = []
