class Board
  ...
  def valid_move_pawns(start)
    valid_moves = []
    row, col = start
    if valid_coordinates?(row, col) && (@grid[row][col] == "\u2659" || @grid[row][col] == "\u265F")
      if @grid[row][col] == "\u2659" #white pawn
        if row == 1 
          valid_moves << [row + 1, col] if @grid[row + 1][col] == " "
          valid_moves << [row + 2, col] if @grid[row + 2][col] == " "
        elsif @grid[row + 1][col] ==" "
          valid_moves << [row + 1, col]
        end
        valid_moves << [row + 1, col - 1]  if @black_listed.include?(@grid[row + 1][col - 1]) 
        valid_moves << [row + 1, col + 1] if @black_listed.include?(@grid[row + 1][col + 1]) 
      end
      if @grid[row][col] == "\u265F" #black pawn
        ...
        valid_moves << [row - 1, col - 1]  if @white_listed.include?(@grid[row - 1][col - 1]) 
        valid_moves << [row - 1, col + 1] if @white_listed.include?(@grid[row - 1][col + 1]) 
      end
      valid_moves
    else
  ...
  def valid_move_queen(start)
    return valid_move_bishop(start) + valid_move_rook(start)
  end

  def valid_move_king(start)
    valid_moves = []
    row, col = start
    possible_moves = [
  ...
  def valid_move(start)
    ...
    when "\u2659","\u265F"
      valid_moves = valid_move_pawns(start)
    when "\u2658", "\u265E"
      valid_moves = valid_move_knight(start)
    when "\u2656", "\u265C"
      valid_moves = valid_move_rook(start)
    when "\u2657", "\u265D"
      valid_moves = valid_move_bishop(start)
    when "\u2655", "\u265B"
      valid_moves = valid_move_queen(start)
    when "\u2654", "\u265A"
      valid_moves = valid_move_king(start)
    end
    valid_moves
  end

  def all_valid_move(color)
    valid_moves = []
    @grid.each_with_index do |row, i|
      row.each_with_index do |cell, j|
        if color == "white" && @white_listed.include?(cell)
          valid_moves = valid_moves + valid_move([i, j])
  ...
  def checkmate?(color)
    ...
    king_moves = valid_move_king(king)
    all_moves = []
    king_moves << king
    if color == "white"
      all_moves = all_valid_move("black")
    else
      all_moves = all_valid_move("white")
    end
